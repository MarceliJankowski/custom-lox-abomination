...
/// Count of distinct states encodable in a single byte.
...
/// @note Deallocation of non-existing objects is permitted for convenience.
...
/// Use `memory_manager` to allocate object of `new_size`.
...
/// Use `memory_manager` to reallocate `object` of `old_size` to `new_size`.
...
/// Use `memory_manager` to deallocate `object` of `old_size`.
...
/// Get `object` byte located at `index`; `index` goes from LSB to MSB.
